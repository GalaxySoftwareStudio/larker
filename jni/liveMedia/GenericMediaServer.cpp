      ...
      fClientSessions(HashTable::create(STRING_HASH_KEYS))
{
  ...
  env.taskScheduler().turnOnBackgroundReadHandling(fServerSocket, incomingConnectionHandler, this); // Arrange to handle connections from others:
}
...
{
  int ourSocket = -1;
  LOGE("Creating socket on port 8554");
  do
  {
    ...
    LOGE("Socket on port 8554 is now open !");
    return ourSocket;
  } while (0);
